...
class Admin {
	...
	api_test_internal_job(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			var job = self.startInternalJob({ title: "Test job that does nothing", username: user.username, type: 'maint' });
			var counter = 0;
	...
	api_admin_run_optimization(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			var job = self.startInternalJob({ title: "Database optimization", username: user.username, type: 'maint' });
	...
	api_admin_import_data(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			var job = self.startInternalJob({ title: "Custom data import", username: user.username, type: 'maint' });
						...
						function(line, callback) {
							...
							if (row.index && row.id && row.record) {
								...
								self.unbase.insert( row.index, row.id, row.record, function(err) {
									if (err) self.logError('import', "Import line #" + line_idx + " failed: " + err);
									callback();
								} ); // unbase.insert
							}
	...
	api_admin_export_data(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			var job = self.startInternalJob({ title: "Custom data export", username: user.username, type: 'maint' });
						...
						self.storage.get( user_path, function(err, user) {
							...
							async.eachSeries( item.avatars ? [64, 256] : [],
								...
								function() {
									...
									stream.write( JSON.stringify({ key: key, value: user }) + "\n", 'utf8', callback );
								}
							); // eachSeries
						} ); // get
					},
			...
			exportTypes.jobFiles = function(item, callback) {
				...
				// item: { query?, max_size?, logs, files }
				if (!item.query) item.query = '*';
	...
	api_admin_delete_data(args, callback) {
		...
		this.loadSession(args, function(err, session, user) {
			...
			var job = self.startInternalJob({ title: "Custom data deletion", username: user.username, type: 'maint' });
			var deleteTypes = {};
			...
			deleteTypes.list = function(item, callback) {
				...
				self.storage.listDelete( item.key, true, function(err) {
					if (err) {
						if (err.code == 'NoSuchKey') self.logDebug(7, "List does not exist, skipping: " + item.key);
						else self.logError('export', "Failed to fetch list header: " + item.key + ": " + err);
					}
